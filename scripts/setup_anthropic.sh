#!/usr/bin/env bash
# Anthropic Claude 백엔드 원샷 셋업 (가장 안정적인 옵션).
# 사용:
#   bash scripts/setup_anthropic.sh
...
echo "==> Anthropic 설정 추가"
echo 'LLM_BACKEND=anthropic' >> "$ENV_FILE"
echo 'ANTHROPIC_MODEL=claude-haiku-4-5-20251001' >> "$ENV_FILE"

echo ""
echo "==> Anthropic API 키 입력 (sk-ant-... 로 시작)"
echo "    console.anthropic.com → API Keys → Create Key"
echo ""
read -r -s -p "Anthropic 키 (입력 안 보임): " ANTHROPIC_KEY
echo ""
...
if [[ "$ANTHROPIC_KEY" != sk-ant-* ]]; then
    echo "WARN: 키가 'sk-ant-' 로 시작하지 않음. 그래도 저장."
fi
...
echo " Haiku 4.5는 보통 2~3초면 결과 도착."
echo "============================================"
